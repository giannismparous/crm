...
export function OrgChartPersonLink({
  ...
  return (
    <button
      ...
      className={`nodrag nopan nowheel cursor-pointer inline-flex max-w-full items-center gap-1.5 rounded-md text-left font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/30 ${
        isYou
          ? "text-base font-bold text-slate-900 decoration-transparent hover:bg-accent/10"
          : "text-accent underline decoration-accent/35 underline-offset-2 hover:bg-accent/5 hover:text-accent-dim"
      } ${className}`.trim()}
    >
      {showAvatar ? <PersonAvatar person={person} size="2xs" className={isYou ? "!ring-2 !ring-accent" : ""} /> : null}
